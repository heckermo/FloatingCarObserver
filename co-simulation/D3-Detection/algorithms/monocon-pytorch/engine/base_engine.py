...
class BaseEngine:
    def __init__(self, 
        ...
        if not is_test:
            exist = False
            assert not(auto_resume and cfg.MODEL.LOAD_MODEL), "Both 'auto_resume' and 'LOAD_MODEL' cannot be True."
            ...
            if cfg.MODEL.LOAD_MODEL is not None:
                assert os.path.isfile(cfg.MODEL.LOAD_MODEL), f"Model file '{cfg.MODEL.LOAD_MODEL}' does not exist."
                self.load_checkpoint(cfg.MODEL.LOAD_MODEL)
                tprint(f"Model is loaded from '{cfg.MODEL.LOAD_MODEL}'.")
    ...
    def train(self, resume_from: str = None) -> None:
        ...
        for epoch in range(self.epochs, self.target_epochs + 1):
            ...
            if (self.val_period > 0) and (epoch % self.val_period == 0):
                ...
                eval_dict = self.evaluate() # TODO check what this function does

                # log all items in the eval_dict together with the current epoch
    ...
    def load_checkpoint(self, 
        ...
        self.epochs = 0
        self.target_epochs = self.cfg.SOLVER.OPTIM.NUM_EPOCHS
