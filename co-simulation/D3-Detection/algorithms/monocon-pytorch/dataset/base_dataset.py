...
try:
    from data_classes import KITTICalibration, KITTIMultiObjects
except:
...
class BaseKITTIMono3DDataset(Dataset):
    def __init__(self, 
        ...
        with open(os.path.join(os.path.join(self.base_root, 'ImageSets'), f'{split}.txt')) as f:
            file_prefix = f.readlines()
        self.file_prefix = [fp.replace('\n', '') for fp in file_prefix]
        sub_root = 'testing' if (split == 'testing') else 'training'
