...
from Co_Simulation.tools.CARLA_KITTI.datadescriptor import KittiDescriptor
from Co_Simulation.tools.CARLA_KITTI.camera_utils import *
from Co_Simulation.tools.CARLA_KITTI.client_bounding_boxes import ClientSideBoundingBoxes
...
#MIN_BBOX_AREA_IN_PX = 25
KITTI_CRITERIA = {'easy': {'min_bounding_box_height': 40, 'occlusion_level': 0, 'truncation_level': 0.15},
                  'moderate': {'min_bounding_box_height': 25, 'occlusion_level': 1, 'truncation_level': 0.3},
                  'hard': {'min_bounding_box_height': 15, 'occlusion_level': 2, 'truncation_level': 0.5}}


# TODO Make computations faster by vectorization
def calculate_occlusion_stats(image, bbox_points, depth_map, max_render_depth, window_width, window_height, draw_vertices=True):
    ...
    for i in range(len(bbox_points)):
        ...
        if max_render_depth > point_depth > 0 and point_in_canvas((y_2d, x_2d),window_height=window_height, window_width=window_width):
            is_occluded = point_is_occluded(
                (y_2d, x_2d), point_depth, depth_map, window_height, window_width)
            if is_occluded:
...
def get_bounding_box_and_refpoint(agent, camera, camera_calibration):
    ...
    """
    bbox_refpoint = np.array([[0, 0, 0, 1]], dtype=np.float64)
...
def create_kitti_datapoint(agent, camera, position, cam_calibration, image, depth_map, player_transform, kitti_criteria, max_render_depth=70, window_width=1920, window_height=1080):
    """
    ...
    if num_visible_vertices >= MIN_VISIBLE_VERTICES_FOR_RENDER: #> num_vertices_outside_camera:
        ...
        crop_boxes_in_canvas(camera_bbox, window_width, window_height)

        bbox_2d = calc_projected_2d_bbox(camera_bbox)

        area = calc_bbox2d_area(bbox_2d)
        height = calc_bbox_height(bbox_2d)
        #if area < args.kitti_detection_parameters.min_bbox_area_in_px:
        #    logging.info("Filtered out bbox with too low area {}".format(area))
        #    return image, None, None

        occlusion = calculate_occlusion(camera_bbox, agent, depth_map, window_width, window_height)
        rotation_y = get_relative_rotation_y(agent, player_transform, position)
        alpha = get_alpha(agent, player_transform, position)
        if alpha < -math.pi or alpha > math.pi or math.isnan(alpha):
            print("Alpha out of range with value: ", alpha)
            return image, None, None
        assert -math.pi <= alpha <= math.pi, "Alpha must be in range [-pi..pi]"
        ...
        datapoint.set_occlusion(occlusion.item())
        datapoint.set_height(height.item())
        datapoint.set_area(area.item())

        obj_type = evaluate_kitti_criteria(obj_type, height, truncation, occlusion, kitti_criteria)
        datapoint.set_type(obj_type)
...
def evaluate_kitti_criteria(obj_type, height, truncation, occlusion, kitti_criteria):
    criteria = KITTI_CRITERIA[kitti_criteria]
    if criteria['min_bounding_box_height'] > height:
        obj_type = 'DontCare'
    if truncation > criteria['truncation_level']:
        obj_type = 'DontCare'
    if occlusion > criteria['occlusion_level']:
        obj_type = 'DontCare'
    return obj_type
    


def calculate_occlusion(bbox, agent, depth_map, window_width, window_height):
...
def get_relative_rotation_y(agent, player_transform, position):
    ...
    ::param agent: The vehicle that is being labeled
    ::param player_transform: The transform of the EGO vehicle
    ::param position: The position of the camera relative to the EGO vehicle coordinate system
    """
    # TODO also allow angles in degrees
    ...
    rel_angle = math.radians(rel_angle)
...
def get_alpha(agent, player_transform, position):
    """
    Returns the angle between the agent and the camera in the horizontal plane
    ::param agent: The vehicle that is being labeled
    ::param player_transform: The transform of the EGO vehicle
    ::param position: The position of the camera relative to the EGO vehicle coordinate system
    """
