...
class CarlaSimulation(object):
    ...
    def __init__(self, host, port, step_length):
        ...
        settings.fixed_delta_seconds = 0.05  # Desired simulation step size
        settings.max_substeps = 5  # Number of substeps
        settings.max_substep_delta_time = 0.01  # Duration of each substep
    ...
    def get_sensor_actors(self) -> list:
        """
        Get all sensor actors in the simulation.
            
        :return: list of sensor actors ids.
        """
        sensor_actors = self.world.get_actors().filter('sensor.*')
        sensor_actors_ids = [sensor.id for sensor in sensor_actors]
        return sensor_actors_ids
    ...
    def destroy_actor(self, actor_id):
        ...
        if actor is not None:
            # destroy the sensors attached to the actor first
            all_actors = self.world.get_actors()
            for sensor in all_actors.filter('sensor.*'):
                if sensor.parent and sensor.parent.id == actor_id:
                    sensor.destroy()
