...
def get_fco_ids(config, sumo_cfg):
    assert sum(config.fco_penetration_rate.__dict__.values()) == 1, 'Sum of penetration rates of different sensor setups must be equal to 1'
    categories = list(config.fco_penetration_rate.__dict__.keys())
    probs = list(config.fco_penetration_rate.__dict__.values())
    # Load the config and get the route file names
    sumo_base_path = '/home/jeremias/Carla_Sumo_detector/Co_Simulation/sumo_files'
    tree = ET.parse(sumo_cfg)
    root = tree.getroot()
    route_files = root.find('input').findall('route-files')[0].get('value').split(',')
    route_files = [route_file for route_file in route_files if 'carlavtypes.rou.xml' not in route_file]
    # Get all vehicle IDs from the route files
    vehicle_ids = []
    for route_file in route_files:
        tree = ET.parse(os.path.join(sumo_base_path, route_file))
        root = tree.getroot()
        for vehicle in root.findall('vehicle'):
            vehicle_ids.append(vehicle.get('id'))
    # Assign FCO IDs based on the penetration rate
    fco_ids = {}
    for vehicle in vehicle_ids:
        sampled_category = np.random.choice(categories, p=probs)
        if sampled_category != 'no_sensor':
            fco_ids[vehicle] = sampled_category
    return fco_ids
