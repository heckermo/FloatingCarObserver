...
class HierarchicalInTrainingEvaluator:
    def __init__(self, path, mode: str = 'raw'):
        """
        Initialize the evaluator.
        ...
        #normalization is alway zscore
            
        with open (os.path.join(path, "mean.npy"), "rb") as m:
            self.mean = np.load(m)

        with open (os.path.join(path, "std.npy"), "rb") as s:
            self.std = np.load(s)
